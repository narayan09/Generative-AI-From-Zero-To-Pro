...
### 👉 What they are
Static or dynamic data exposed to the LLM.
...
| Tools | Resources |
| ----- | --------- |
| Perform actions | Provide data |
| Function call | Data access |

**Example:**
- **Tool** → `search_docs(query)`
- **Resource** → `docs folder`
...
1. User asks: *“Weather in Pune”*
2. **MCP Client (LLM)** reads available tools
3. **LLM chooses** the `get_weather` tool
4. Client sends request to **MCP Server**
...
## 🧠 Simple Analogy (Helps in Interview)
...
> The **Manager (LLM)** decides what to do, the **Factory (Server)** hosts the machines, **Machines (Tools)** perform actions, and **Raw materials (Resources)** provide data.

---

## 🔥 Interview Quick Summary

**Q:** *“Explain MCP architecture.”*

**A:**  
> *“MCP consists of a client, server, tools, and resources. The client (LLM) decides which tool to use, the server exposes tools with a standardized schema, tools perform actions, and resources provide data. This separation enables scalable and modular AI systems.”*
...
| Component | Role |
| --------- | ---- |
| Client    | Brain (LLM decision maker) |
| Server    | Tool manager |
| Tools     | Action layer (functions) |
